...
class HealthDataControllerMvcTest {
    ...
    @MockBean HealthDataService healthDataService;
    ...
    void createSleep_shouldReturn200() throws Exception {
        when(healthDataService.createSleep(any())).thenReturn(SleepResponse.builder()
                .id(UUID.randomUUID()).durationMin(480).quality(4).build());
    ...
    void deleteSleep_ownEntry_shouldReturn204() throws Exception {
        ...
        mockMvc.perform(delete("/api/v1/health/sleep/" + UUID.randomUUID())
                        .header("Authorization", "Bearer " + jwt()))
    ...
    void deleteSleep_otherUser_shouldReturn403() throws Exception {
        doThrow(new ForbiddenException("Not your entry"))
                .when(healthDataService).deleteSleep(any());
    ...
    void getSleepStats_shouldReturn200() throws Exception {
        when(healthDataService.getSleepStats()).thenReturn(
                SleepStatsDto.builder()
                        .entryCount(8)
        ...
        mockMvc.perform(get("/api/v1/health/sleep/stats")
                        .header("Authorization", "Bearer " + jwt()))
                .andExpect(status().isOk());
    }
    ...
    void createWeight_shouldReturn200() throws Exception {
        when(healthDataService.createWeight(any())).thenReturn(WeightResponse.builder()
                .id(UUID.randomUUID()).weightKg(new BigDecimal("75.5")).build());

        mockMvc.perform(post("/api/v1/health/weight")
                        .header("Authorization", "Bearer " + jwt())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
    ...
    void addWater_shouldReturn200() throws Exception {
        when(healthDataService.addWater(any())).thenReturn(
                WaterResponse.builder().id(UUID.randomUUID()).amountMl(250).build());

        mockMvc.perform(post("/api/v1/health/water")
                        .header("Authorization", "Bearer " + jwt())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
    ...
    void getWaterToday_shouldReturn200() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/health/water/today")
                        .header("Authorization", "Bearer " + jwt()))
                .andExpect(status().isOk())
    ...
    void syncActivity_shouldReturn200() throws Exception {
        when(healthDataService.syncActivity(any())).thenReturn(ActivityEntryDto.builder()
                .steps(8000).caloriesBurned(300).build());

        mockMvc.perform(post("/api/v1/health/activity/sync")
                        .header("Authorization", "Bearer " + jwt())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
    ...
    void getDashboard_shouldReturn200() throws Exception {
        when(healthDataService.getDashboard()).thenReturn(DashboardDto.builder()
                .waterTodayMl(1500).stepsTodayCount(7000).build());

        mockMvc.perform(get("/api/v1/health/dashboard")
                        .header("Authorization", "Bearer " + jwt()))
                .andExpect(status().isOk())
    ...
    void getCyclePrediction_noData_shouldReturn404() throws Exception {
        when(healthDataService.getCyclePrediction())
                .thenThrow(new ResourceNotFoundException("Cycle", "any", ""));

        mockMvc.perform(get("/api/v1/health/cycle/prediction")
                        .header("Authorization", "Bearer " + jwt()))
                .andExpect(status().isNotFound());
    }
    ...
    void createSymptom_shouldReturn200() throws Exception {
        when(healthDataService.createSymptom(any())).thenReturn(SymptomResponse.builder()
                .id(UUID.randomUUID()).symptom("Headache").intensity(7).build());

        mockMvc.perform(post("/api/v1/health/symptoms")
                        .header("Authorization", "Bearer " + jwt())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
