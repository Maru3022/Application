...
class HealthDataServiceCriticalTest {
    ...
    void createSleep_shouldCalculateDuration() {
        ...
        SleepRequest req = SleepRequest.builder()
                .sleepStart(start).sleepEnd(end).quality(4).source("manual").build();
    ...
    void createSleep_endBeforeStart_shouldCalculateNegativeDuration() {
        ...
        SleepRequest req = SleepRequest.builder()
                .sleepStart(start).sleepEnd(end).quality(3).build();
    ...
    void deleteSleep_otherUserEntry_shouldThrowForbidden() {
        ...
        assertThatThrownBy(() -> healthDataService.deleteSleep(entryId))
    ...
    void addWater_andGetToday_shouldSum() {
        ...
        healthDataService.addWater(WaterRequest.builder().amountMl(250).recordedAt(now).build());
        healthDataService.addWater(WaterRequest.builder().amountMl(500).recordedAt(now).build());
    ...
    void createSymptom_shouldPersist() {
        SymptomRequest req = SymptomRequest.builder()
                .symptom("Headache").intensity(7).recordedAt(OffsetDateTime.now()).notes("After lunch").build();
    ...
    void getCyclePrediction_noData_shouldThrow() {
        assertThatThrownBy(() -> healthDataService.getCyclePrediction())
                .isInstanceOf(ResourceNotFoundException.class);
    }
    ...
    void syncActivity_shouldUpsert() {
        ActivityEntryDto dto = ActivityEntryDto.builder()
                .date(LocalDate.now()).steps(5000).caloriesBurned(200).source("manual").build();
        ...
        ActivityEntryDto update = ActivityEntryDto.builder()
                .date(LocalDate.now()).steps(10000).source("manual").build();
