...
class MentalServiceCriticalTest {
    ...
    @Autowired private MentalService mentalService;
    ...
    @Autowired private MeditationRepository meditationRepository;
    @Autowired private BreathingSessionRepository breathingSessionRepository;
    ...
    void createMood_shouldPersist() {
        MoodRequest req = MoodRequest.builder()
                .moodScore(8).emotions(List.of("happy", "energetic")).note("Great day!").recordedAt(OffsetDateTime.now()).build();
    ...
    void createMood_nullEmotions_shouldWork() {
        MoodRequest req = MoodRequest.builder()
                .moodScore(5).recordedAt(OffsetDateTime.now()).build();
        MoodResponse resp = mentalService.createMood(req);
    ...
    void createJournal_shouldPersist() {
        JournalRequest req = JournalRequest.builder()
                .content("Today I reflected on my goals").tags(List.of("reflection", "goals")).recordedAt(OffsetDateTime.now()).build();
    ...
    void createStress_shouldPersist() {
        StressRequest req = StressRequest.builder().level(7).notes("Deadline approaching").recordedAt(OffsetDateTime.now()).build();
    ...
    void getMeditations_withCategory_shouldFilter() {
        meditationRepository.save(Meditation.builder().title("Calm").category("sleep").durationMin(10).build());
        meditationRepository.save(Meditation.builder().title("Focus").category("focus").durationMin(15).build());
    ...
    void getMeditations_noCategory_shouldReturnAll() {
        meditationRepository.save(Meditation.builder().title("Calm").category("sleep").build());
        meditationRepository.save(Meditation.builder().title("Focus").category("focus").build());
    ...
    void createBreathingSession_shouldPersist() {
        BreathingSessionRequest req = BreathingSessionRequest.builder()
                .technique("4-7-8").durationMin(5).build();
    ...
    void moodHistory_shouldReturnOrdered() {
        mentalService.createMood(MoodRequest.builder().moodScore(5).recordedAt(OffsetDateTime.now().minusHours(2)).build());
        mentalService.createMood(MoodRequest.builder().moodScore(8).recordedAt(OffsetDateTime.now()).build());
